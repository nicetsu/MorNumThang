...
- ทำตาม "แนวทาง" ที่ให้มา:
  - ถ้าแนวทาง = รับยาฟรี และ "รับยาฟรีได้ = ใช่" ให้บอกว่าอาการนี้ไปรับยาฟรีที่ร้านยาคุณภาพที่เข้าร่วมได้เลย ระบุชื่ออาการด้วย
...
- severity (ความควรใส่ใจ 0-10) ให้ประเมินจาก "ควรทำอะไรต่อ":
  - 0-3 = เรื่องทั่วไป ข่าวดี หรืออาการเดิมที่คงที่/หายแล้ว ดูแลเองที่บ้านได้ (เรื่องดีให้ 0-2)
  - 4-7 = มีการเปลี่ยนแปลงหรือแย่ลงจากเดิม หรือเป็นต่อเนื่องหลายวัน ควรเฝ้าดู (เช่น กิน/นอน/เดินเปลี่ยนไปจากปกติ)
  - 8-10 = รุนแรงหรือฉุกเฉิน ควรพาไปพบแพทย์ — รวมถึงอุบัติเหตุ/การบาดเจ็บ, หายใจลำบาก, เจ็บรุนแรง, และภาวะรู้สึกตัวเปลี่ยน (ซึม สับสน เรียกไม่รู้เรื่อง) โดยเฉพาะในผู้สูงอายุ
  - ถ้าเรื่องที่เล่าไม่เกี่ยวกับสุขภาพหรือการดูแลเลย ให้ severity = 0
...
export async function organizeNarrative(story: string): Promise<OrganizedItem[]> {
  const { text } = await generateText({
    ...
    prompt: story,
    temperature: 0, // greedy — same story → same category/severity (เดิม 0.2 ทำให้ผลไม่นิ่ง รันซ้ำได้คนละหมวด)
  });
